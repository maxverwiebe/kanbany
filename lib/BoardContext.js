...
export const BoardProvider = ({ children }) => {
  ...
  const [cards, setCards] = useState([
    { id: "1", text: "Task 1", columnId: "todo", labels: ["1"], dueDate: null },
    {
      id: "2",
      text: "Task 2",
  ...
  const addCard = (columnId, text) => {
    ...
    setCards((prev) => [...prev, { id, text, columnId, labels: [], dueDate: null }]);
    return id;
  };
  ...
  const importBoard = async (jsonData) => {
    try {
      ...
      if (data.columns && data.cards && data.labels) {
        ...
        const cardsWithDefaults = data.cards.map((card) => ({
          dueDate: null,
          ...card,
        }));
